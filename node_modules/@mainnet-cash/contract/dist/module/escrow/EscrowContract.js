import { CONST, Mainnet, fromUtxoId } from "mainnet-js";
...
export class EscrowContract extends Contract {
     ...
     */
    constructor({ sellerAddr, buyerAddr, arbiterAddr, amount, nonce, }) {
        ...
        [this.sellerAddr, this.buyerAddr, this.arbiterAddr] = addressArgs.map((x) => Mainnet.sanitizeAddress(x));
        this.amount = amount;
    }
    /**
     ...
     * @returns A new contract
     */
    static create({ sellerAddr, buyerAddr, arbiterAddr, amount, nonce, }) {
     ...
     * @returns A new contract
     */
    async call(wif, funcName, outputAddress, getHexOnly = false, utxoIds) {
        if (!outputAddress) {
            if (funcName === "spend") {
    ...
    getSerializedArguments() {
        ...
        return Mainnet.btoa(args.map((a) => Mainnet.btoa(a.toString())).join(CONST.DELIMITER));
    }
    /**
     ...
     */
    static fromId(escrowContractId) {
        const [type, network, serializedArgs, serializedParams, serializedScript, nonce,] = escrowContractId.split(CONST.DELIMITER);
        const [sellerAddr, buyerAddr, arbiterAddr, amount] = Mainnet.atob(serializedArgs)
     ...
     */
    static escrowContractFromJsonRequest(request) {
    ...
    async _sendMax(wif, funcName, outputAddress, getHexOnly = false, utxoIds) {
        ...
        if (utxos.length > 0) {
            try {
                ...
                if (this.amount > amount) {
                    throw Error(`The contract amount (${this.amount}) could not be submitted for a tx fee (${fee}) with the available with contract balance (${balance})`);
                }
